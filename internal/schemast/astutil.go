...
func combineUnsupported(err error, feature string) error {
	return errors.Join(err, fmt.Errorf("schemast: unsupported feature %s", feature))
}
...
func strSliceLit(s []string) ast.Expr {
	c := &ast.CompositeLit{
		Type: &ast.ArrayType{
...
func bitORLit(lits ...ast.Expr) ast.Expr {
	if len(lits) == 1 {
		return lits[0]
	}
